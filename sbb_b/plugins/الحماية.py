...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق ستوك ثون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@sbb_b.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق ستوك ثون
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق ستوك ثون
...
async def approve_p_m(event):
    ...
    if not reason:
        reason = "لم يذكر"
    try:  # ترجمه وكتابة فريق ستوك ثون
        PM_WARNS = sql.get_collection("pmwarns").json
    except AttributeError:
        PM_WARNS = {}
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية ستوك ثون.txt",
        caption="قائمة المسموح لهم الحالية\n سورس ستوك ثون العربي \n @jmthon",
    )  # ترجمه وكتابة فريق ستوك ثون
